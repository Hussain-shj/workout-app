...
import { ArrowLeft, Check, Dumbbell, ExternalLink, History, Home, LogOut, Play, Plus, RefreshCw, Save, UserRound, X } from 'lucide-react'
...
function App() {
  ...
  const updateLiveExercise=(updated:WorkoutExercise)=>{
    ...
    setProgram(prev=>prev?{...prev,days:prev.days.map(day=>({...day,exercises:day.exercises.map(pe=>pe.exercise.id===selectedWorkoutExercise?.exercise_id?{...pe,exercise:{...pe.exercise,id:updated.exercise_id,name_en:updated.name,name_ar:updated.name_ar,primary_muscle:updated.primary_muscle||pe.exercise.primary_muscle,movement_pattern:updated.movement_pattern,youtube_url:updated.youtube_url},target_rep_min:updated.target_rep_min,target_rep_max:updated.target_rep_max}:pe)}))}:prev)
  }
...
function DayPage({day,activeWorkout,onBack,onOpenExercise,onCompleteWorkout}:{day:ProgramDay;activeWorkout:StartedWorkout|null;onBack:()=>void;onOpenExercise:(pe:ProgramExercise)=>void;onCompleteWorkout:()=>Promise<void>}){const[busy,setBusy]=useState(false);return <section><PageHeader title={day.name} subtitle={`${day.exercises.length} exercises`} onBack={onBack}/><div className="day-exercise-list">{day.exercises.map((pe)=>{const live=activeWorkout?.exercises.find(e=>e.exercise_id===pe.exercise.id);const displayName=live?.name||pe.exercise.name_en;return <button className="exercise-list-row image-row" key={pe.id} onClick={()=>onOpenExercise(pe)}><Img name={displayName} className="exercise-thumb"/><div className="exercise-list-copy"><strong>{displayName}</strong>{(live?.name_ar||pe.exercise.name_ar)&&<span>{live?.name_ar||pe.exercise.name_ar}</span>}<small>{pe.target_sets} sets · {pe.target_rep_min}-{pe.target_rep_max} reps · RIR {pe.target_rir??2}</small></div><span className="open-label">Open</span></button>})}</div><button className="primary-button full finish-button" disabled={busy} onClick={async()=>{setBusy(true);try{await onCompleteWorkout()}finally{setBusy(false)}}}><Check size={18}/>{busy?'Completing...':'Complete Workout'}</button></section>}
...
function ExercisePage({workout,exercise,onBack,onCompleted,onExerciseUpdated}:{workout:StartedWorkout;exercise:WorkoutExercise;onBack:()=>void;onCompleted:()=>void;onExerciseUpdated:(exercise:WorkoutExercise)=>void}){
  const makeRow=():SetRow=>({weight:'',reps:'',rir:String(exercise.target_rir??2)})
  const[rows,setRows]=useState<SetRow[]>(()=>Array.from({length:exercise.target_sets},makeRow))
  ...
  const updateRow=(i:number,f:'weight'|'reps'|'rir',v:string)=>{setRows(p=>p.map((r,x)=>x===i?{...r,[f]:v}:r));setError('')}
  const addSet=()=>{setRows(prev=>[...prev,{weight:'',reps:'',rir:String(exercise.target_rir??2)}]);setError('')}
  const saveSet=async(i:number)=>{
    const r=rows[i]
    const weight=r.weight.trim(); const reps=r.reps.trim(); const rir=r.rir.trim()
    if(!weight||!reps||!rir){setError(`Set ${i+1}: enter KG, reps and RIR before saving.`);return}
    const weightNum=Number(weight), repsNum=Number(reps), rirNum=Number(rir)
    if(!Number.isFinite(weightNum)||weightNum<0){setError(`Set ${i+1}: enter a valid weight.`);return}
    if(!Number.isInteger(repsNum)||repsNum<=0){setError(`Set ${i+1}: reps must be at least 1.`);return}
    if(!Number.isFinite(rirNum)||rirNum<0||rirNum>10){setError(`Set ${i+1}: RIR must be between 0 and 10.`);return}
    if(r.saved)return
    setError('')
    try{await api(`/workouts/${workout.workout_session_id}/sets`,{method:'POST',body:JSON.stringify({exercise_session_id:exercise.exercise_session_id,weight_kg:weightNum,reps:repsNum,rir:rirNum,completed:true})});setRows(p=>p.map((x,n)=>n===i?{...x,saved:true}:x))}catch(e){setError(e instanceof Error?e.message:'Unable to save set')}
  }
  ...
  const completeExercise=async()=>{if(!rows.some(r=>r.saved)){setError('Save at least one set before completing the exercise.');return}await saveNote();onCompleted()}
  ...
  return <section><PageHeader title={exercise.name} subtitle={`${exercise.target_sets} target sets · ${exercise.target_rep_min}-${exercise.target_rep_max} reps`} onBack={onBack}/>{error&&<div className="error-banner workout-alert">{error}</div>}{message&&<div className="success-banner workout-alert">{message}</div>}
    ...
    <div className="exercise-card single-exercise-card"><div className="set-table-head"><span>SET</span><span>KG</span><span>REPS</span><span>RIR</span><span/></div>{rows.map((r,i)=><div className="set-row" key={i}><span>{i+1}</span><input inputMode="decimal" value={r.weight} onChange={e=>updateRow(i,'weight',e.target.value)} placeholder="KG" disabled={r.saved}/><input inputMode="numeric" value={r.reps} onChange={e=>updateRow(i,'reps',e.target.value)} placeholder="Reps" disabled={r.saved}/><input inputMode="decimal" value={r.rir} onChange={e=>updateRow(i,'rir',e.target.value)} placeholder="RIR" disabled={r.saved}/><button className={r.saved?'set-done saved':'set-done'} onClick={()=>saveSet(i)} disabled={r.saved}>{r.saved?'✓':'+'}</button></div>)}
      <button type="button" className="add-set-button" onClick={addSet}><Plus size={17}/> Add Set</button>
      <div className="exercise-notes"><label>Exercise notes</label><textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="How did this exercise feel? Pain, fatigue, form, difficulty..." maxLength={2000}/><button className="note-save" onClick={saveNote}><Save size={15}/>{noteSaved?'Saved':'Save note'}</button></div></div>
